<?php
    ...
    $view = "SELECT * from post p LEFT JOIN comment c on p.id = c.id_post_comment WHERE p.id = :id ORDER BY created_comment DESC";
...
while($blog = $prepare->fetch(mode: PDO :: FETCH_ASSOC)){
    ...
    ?>
        ...
        <form action="function/comment.php" method="post">
            ...
            <input type="hidden" name="id_comment" value="<?=($blog['id'])?>">
